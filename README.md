...
默认可写范围为**当前任务执行时被注入的项目目录 + `/tmp` + `$TMPDIR`**，`/dev/null` 始终合法（项目目录由宿主在任务执行时注入，其来源与局限见[已知局限](#已知局限)第 4 条）。「额外可写根」（`extra_writable_roots`，默认为空）中声明的目录会**追加**到该范围——不配置则不生效，仅在项目含多个文件夹、默认范围不足时才需要：

- 可写范围内的文件写入：放行；在需要确认的权限模式下，同时跳过宿主的例行询问
- 可写范围外的文件写入（桌面、`home` 目录、其他项目）：转为人工确认
