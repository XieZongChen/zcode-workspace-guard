...
- 允许写入：当前工作区根目录、`/tmp`、`$TMPDIR`
- 越界写入（家目录、系统路径、其他项目）会触发 `ask` 人工确认——确认框
  就是用户的审批通道：用户批准则这一次放行，拒绝则你必须换方案
- 收到"写入目标在工作区外"时，首选做法是**改用工作区内路径**（如把
  输出写到工作区下的临时文件）；确需越界时向用户说明理由，由确认框决定
- Bash 中 `>` 重定向、`cp`/`mv`/`rm`/`tee` 作用于工作区外绝对路径同样
  会被检出（启发式）
